...
from files.models import RecommendRequest, RecommendResponse, UserStatusResponse
from files.data_loader import data_store
from files.recommenders import RecommenderEngine

app = FastAPI(title="Hybrid Recommendation System", description="Amazon product recommendations", version="2.0.0")
...
def health_check():
    return {"status": "healthy","models_loaded": {
            "svd": data_store.best_svd is not None,
            "svdpp": data_store.svdpp is not None,
            "knn": data_store.knn is not None,}}
...
async def recommend(req: RecommendRequest):
    ...
    if cf_eligible:
        recs = await loop.run_in_executor(executor, RecommenderEngine.get_recommendations,
            req.user_id,req.k,)
    else:
